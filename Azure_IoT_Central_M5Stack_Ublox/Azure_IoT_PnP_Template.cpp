...
/* --- Defines --- */
#define AZURE_PNP_MODEL_ID "dtmi:arkaIotCentral:gpsM5stack_4hd;1"
...
#define SAMPLE_MANUFACTURER_PROPERTY_VALUE             "m5stack core"
#define SAMPLE_MODEL_PROPERTY_VALUE                    "M5Stack with Ublox Neo 6m "
#define SAMPLE_VERSION_PROPERTY_VALUE                  "1.0.0"
#define SAMPLE_OS_NAME_PROPERTY_VALUE                  "FreeRTOS"
#define SAMPLE_ARCHITECTURE_PROPERTY_VALUE             "ESP3"
...
#define TELEMETRY_PROP_NAME_LAT                        "lat"
#define TELEMETRY_PROP_NAME_LON                        "lon"
#define TELEMETRY_PROP_NAME_ALT                        "alt"
...
#define DOUBLE_DECIMAL_PLACE_DIGITS 5
...
static int generate_telemetry_payload(
  uint8_t* payload_buffer, size_t payload_buffer_size, size_t* payload_buffer_length,TinyGPSPlus* tinygps);
...
int azure_pnp_send_telemetry(azure_iot_t* azure_iot, TinyGPSPlus* tinygps)
{
           ...
           difftime(now, last_telemetry_send_time) >= telemetry_frequency_in_seconds)
  {
    ...
    if (generate_telemetry_payload(data_buffer, DATA_BUFFER_SIZE, &payload_size,tinygps) != RESULT_OK)
    {
...
static int generate_telemetry_payload(uint8_t* payload_buffer, size_t payload_buffer_size, size_t* payload_buffer_length ,TinyGPSPlus* tinygps)
{
  az_json_writer jw;
  az_result rc;
  ...
  { 
    ...
    rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR("deliverytracking"));
    EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed setting telemetry json root property  deliverytracking.");

    rc = az_json_writer_append_begin_object(&jw);
    EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed setting telemetry json object  deliverytracking. ");
  
    rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_LAT));
    EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding lat property name to telemetry payload.");
    rc = az_json_writer_append_double(&jw, tinygps->location.lat(), DOUBLE_DECIMAL_PLACE_DIGITS);
    EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding lat property value to telemetry payload. ");
  
    rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_LON));
    EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding lon property name to telemetry payload.");
    rc = az_json_writer_append_double(&jw, tinygps->location.lng(), DOUBLE_DECIMAL_PLACE_DIGITS);
    EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding lon property value to telemetry payload. ");
  
    rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_ALT));
    EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding alt property name to telemetry payload.");
    rc = az_json_writer_append_double(&jw, tinygps->altitude.meters(), DOUBLE_DECIMAL_PLACE_DIGITS);
    EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding alt property value to telemetry payload.");

    rc = az_json_writer_append_end_object(&jw);
    EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed closing telemetry deliverytracking json object.");
  ...
  LogError("Location is not Valid");
  return RESULT_ERROR;
}
